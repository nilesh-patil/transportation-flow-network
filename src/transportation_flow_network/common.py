...
def load_edges_period() -> pd.DataFrame:
    return pd.read_parquet(C.EDGES_PERIOD_PARQUET)


def load_edges_annual() -> pd.DataFrame:
    """Annual (origin, dest) edge list with trip count and weighted means."""
    return pd.read_parquet(C.EDGES_PARQUET)


def load_nodes() -> pd.DataFrame:
    return pd.read_parquet(C.NODES_PARQUET)
